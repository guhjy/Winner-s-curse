  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("num_simulations", "模擬次數:",
                   value = 10000, min = 100, max = 50000, step = 100), # 預設值改為 10000
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("概念解釋",
                 ...
                 p(em("在情境一 (對立假設 H_A 為真) 中："), "模擬結果會估計實際的功效（顯示為「原始研究顯著的比例」），這個值應接近側邊欄顯示的「計算出的統計功效」。對於那些原始研究顯著的案例，「整體重複成功率」也應該接近研究本身的功效。"),
                 ...
                 # *** 新增解釋 ***
                 p(strong("關於使用第一項研究的觀察效應來規劃第二項研究："), "您提到如果第二項研究（重複研究）的設計（例如樣本量計算）是基於第一項研究*觀察到*的效應大小，那麼是否就不存在向平均值迴歸？這是一個重要的區別點。"),
                 tags$ul(
                    tags$li("本模擬的設計：在這個模擬中，「原始研究」和「重複研究」都是基於同一個*真實*的母體效應大小（由側邊欄設定）來生成的。我們觀察到向平均值迴歸，是因為我們首先挑選出那些因機率而觀察到較大效應（並因此達到顯著）的「原始研究」（即「贏家」），然後觀察其對應的「重複研究」。由於重複研究也是基於*真實*效應大小生成的，其結果自然更可能接近真實平均值，而不是第一次觀察到的極端值。"),
                    tags$li("基於觀察效應規劃：如果研究者*真的*採用第一項研究觀察到的（可能因贏家詛咒而被高估的）效應大小來計算第二項研究的樣本量，那麼他們實際上是在為一個可能被高估的效應來設計研究。這並不意味著向平均值迴歸這個現象消失了，而是說研究設計本身可能受到了第一次觀測偏差的影響。如果第一次的效應被高估，基於此規劃的重複研究可能會功效不足（underpowered）以檢測*真實*的效應大小。因此，雖然重複研究的結果*相對於其設計目標*（即第一次觀察到的高估效應）可能沒有顯著「迴歸」，但它仍然會從第一次的極端值向*真實*的平均值迴歸。"),
                    tags$li("總結：本模擬展示了向平均值迴歸，因為重複研究是基於真實效應生成的。若重複研究的規劃基於原始研究的觀察效應，則迴歸現象的體現方式不同，且可能導致重複研究的功效問題。")
                 ),
                 # *** 修改文字 ***
                 p(strong("注意："), "重複研究基於原始研究觀察到的效應大小進行模擬。"),
...
server <- function(input, output, session) {
  ...
  simulation_results <- eventReactive(input$run_sim, {
    ...
    alpha <- input$alpha
    num_sim <- input$num_simulations # 使用輸入框的值
    true_effect_H0 <- 0
    sd_val <- 1
  ...
  output$p_value_distribution_plot <- renderPlot({
    ...
    if (nrow(plot_data) > 0) { # Add check to ensure plot_data is not empty
        plot_data$Hypothesis <- factor(plot_data$Hypothesis, levels = c("對立假設 (H_A) 為真", "虛無假設 (H_0) 為真"))
    } else {
        # Handle empty data case, maybe return an empty plot or a message
    ...
    ggplot(plot_data, aes(x = p_value, fill = Hypothesis)) +
      ...
      scale_fill_manual(values = c("對立假設 (H_A) 為真" = "skyblue", "虛無假設 (H_0) 為真" = "salmon"),
                        # 處理只有一種假設數據的情況
                        limits = levels(plot_data$Hypothesis),
                        drop = FALSE) + # 不要丟棄未使用的因子水平
      labs(
