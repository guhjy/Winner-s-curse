...
run_study <- function(true_effect, n, sd = 1) {
  ...
  if (!is.na(p_value)) {
    ...
    mean2 <- mean(group2)
    var1 <- var(group1)
    # Corrected typo: should be var(group2)
    var2 <- var(group2)
    # 計算合併標準差
    # 檢查變異數是否有效
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("重複成功率與分層比較",
                 ...
                 verbatimTextOutput("stratified_results_HA"), # 此處將包含整體和分層結果
                 hr(),
                 h4("情境二：虛無假設 (H_0) 為真 (無真實效應)"),
                 p("比較不同初始偽陽性 p 值區間的重複成功率 (重複研究 p < alpha 的比例，即偽陽性重複率)："),
                 verbatimTextOutput("stratified_results_H0") # 此處將包含整體和分層結果
        ),
        tabPanel("概念解釋",
                 h4("關於此模擬"),
                 ...
                 # *** 修改：更明確地解釋重複研究的設定 ***
                 p(strong("重要修改："), "在此版本中，每個「重複研究」的數據生成是基於其配對的「原始研究」所 *觀察到* 的效應大小 (Cohen's d)，而不是基於預設的真實效應大小。此外，重複研究使用與原始研究", strong("相同的研究設計 (獨立樣本 t 檢定)、樣本數 (n) 和顯著水準 (alpha)"), "。這模擬了一種情況，即研究者根據初步發現來設計或預期重複研究的結果。如果原始研究的效應量無法計算，則退回使用預設的真實效應大小。"),
                 ...
                 p(em("在「重複成功率與分層比較」分頁中："), "觀察初始 p 值與重複成功率的關係。在 H_A 下，關係可能更複雜。在 H_0 下，由於重複研究基於原始的隨機效應，重複率可能不再恆等於 alpha，且可能與原始 p 值（或其對應的觀察效應量）相關。"),
  ...
  simulation_results <- eventReactive(input$run_sim, {
    ...
    withProgress(message = '模擬進行中', value = 0, {
      for (i in 1:num_sim) {
        ...
        # 3. 執行重複研究，使用從原始研究確定的效應大小，以及原始研究的樣本數和 alpha
        #    (樣本數 n 和 alpha 已經在 run_study 函數和後續分析中使用了)
    ...
    output_HA_text <- ""
    output_stratified_HA <- "" # 將整體重複率也放在這裡
    ...
    if (num_significant_HA > 0) {
      # 整體重複率 (移到 stratified_results_HA 輸出)
      rate_rep_HA <- mean(significant_originals_HA$p_replication < alpha) * 100
      output_stratified_HA <- paste0(output_stratified_HA, sprintf("整體重複成功率 (給定原始研究顯著)：%.2f%%\n", rate_rep_HA))
      output_stratified_HA <- paste0(output_stratified_HA, "  (注意：重複研究基於原始觀察效應量模擬)\n\n")
      ...
      # Removed sections related to Regression Toward the Mean for H_A

      # 分層重複成功率計算 (H_A)
      # *** 修改：調整 p 值分層區間 ***
      bin1_HA <- significant_originals_HA[significant_originals_HA$p_original >= 0.01 & significant_originals_HA$p_original < 0.05, ]
      bin2_HA <- significant_originals_HA[significant_originals_HA$p_original >= 0.005 & significant_originals_HA$p_original < 0.01, ] # Changed from 0.001
      bin3_HA <- significant_originals_HA[significant_originals_HA$p_original < 0.005, ] # Changed from < 0.001

      output_stratified_HA <- paste0(output_stratified_HA, "按原始 p 值分層的重複成功率:\n")
      if (nrow(bin1_HA) > 0) {
      ...
      if (nrow(bin2_HA) > 0) {
        ...
        output_stratified_HA <- paste0(output_stratified_HA, sprintf(" - 原始 p 值介於 [0.005, 0.01) (共 %d 次): 重複成功率 = %.2f%%\n", nrow(bin2_HA), rate2))
      } else { output_stratified_HA <- paste0(output_stratified_HA, " - 原始 p 值介於 [0.005, 0.01): 無此情況。\n") }
      if (nrow(bin3_HA) > 0) {
        rate3 <- mean(bin3_HA$p_replication < alpha) * 100
        output_stratified_HA <- paste0(output_stratified_HA, sprintf(" - 原始 p 值 < 0.005 (共 %d 次): 重複成功率 = %.2f%%\n", nrow(bin3_HA), rate3))
      } else { output_stratified_HA <- paste0(output_stratified_HA, " - 原始 p 值 < 0.005: 無此情況。\n") }

    } else {
      output_HA_text <- paste0(output_HA_text, "\n在對立假設情境下，無顯著的原始研究可供分析重複性/贏家詛咒。\n")
      output_stratified_HA <- paste0(output_stratified_HA, "在對立假設情境下，無顯著的原始研究可供分層分析重複性。\n")
    }
    ...
    output_H0_text <- ""
    output_stratified_H0 <- "" # 將整體偽陽性重複率也放在這裡
    ...
    if (num_significant_H0 > 0) {
      # 偽陽性重複率 (移到 stratified_results_H0 輸出)
      rate_rep_H0 <- mean(significant_originals_H0$p_replication < alpha) * 100
      output_stratified_H0 <- paste0(output_stratified_H0, sprintf("整體偽陽性重複率 (給定原始研究為偽陽性)：%.2f%%\n", rate_rep_H0))
      output_stratified_H0 <- paste0(output_stratified_H0, "  (注意：重複研究基於原始偽陽性觀察效應量模擬)\n\n")
      ...
      # Removed sections related to Regression Toward the Mean for H_0

      # 分層偽陽性重複率計算 (H_0)
      # *** 修改：調整 p 值分層區間 ***
      bin1_H0 <- significant_originals_H0[significant_originals_H0$p_original >= 0.01 & significant_originals_H0$p_original < 0.05, ]
      bin2_H0 <- significant_originals_H0[significant_originals_H0$p_original >= 0.005 & significant_originals_H0$p_original < 0.01, ] # Changed from 0.001
      bin3_H0 <- significant_originals_H0[significant_originals_H0$p_original < 0.005, ] # Changed from < 0.001

      output_stratified_H0 <- paste0(output_stratified_H0, "按原始 p 值分層的偽陽性重複率:\n")
      if (nrow(bin1_H0) > 0) {
      ...
      if (nrow(bin2_H0) > 0) {
        ...
        output_stratified_H0 <- paste0(output_stratified_H0, sprintf(" - 原始 p 值介於 [0.005, 0.01) (共 %d 次): 偽陽性重複率 = %.2f%%\n", nrow(bin2_H0), rate2_H0))
      } else { output_stratified_H0 <- paste0(output_stratified_H0, " - 原始 p 值介於 [0.005, 0.01): 無此偽陽性情況。\n") }
      if (nrow(bin3_H0) > 0) {
        rate3_H0 <- mean(bin3_H0$p_replication < alpha) * 100
        output_stratified_H0 <- paste0(output_stratified_H0, sprintf(" - 原始 p 值 < 0.005 (共 %d 次): 偽陽性重複率 = %.2f%%\n", nrow(bin3_H0), rate3_H0))
      } else { output_stratified_H0 <- paste0(output_stratified_H0, " - 原始 p 值 < 0.005: 無此偽陽性情況。\n") }
      output_stratified_H0 <- paste0(output_stratified_H0, sprintf("   (注意：在 H0 下，重複率基於原始觀察效應量，可能不再等於 alpha = %.1f%%)\n", alpha * 100))

    } else {
      output_H0_text <- paste0(output_H0_text, "\n在虛無假設情境下，無顯著的（偽陽性）原始研究。\n")
      output_stratified_H0 <- paste0(output_stratified_H0, "在虛無假設情境下，無偽陽性原始研究可供分層分析重複性。\n")
    }
