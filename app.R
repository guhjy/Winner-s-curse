...
library(lifecycle) # To manage warnings like the ggplot2 one
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      # *** CHANGE: Default simulation times changed to 10000 ***
      numericInput("num_simulations", "模擬次數:",
                   value = 10000, min = 100, max = 50000, step = 100), # Default set to 10000
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("概念解釋",
                 ...
                 # *** ADDED EXPLANATION ***
                 p(strong("關於此模擬的設計："), "此模擬中，重複研究是基於第一次研究 *觀察到* 的效應量來進行模擬。這代表重複研究的 *設計* 已經考慮了第一次研究的（可能被高估的）結果。因此，向平均值迴歸這個現象，在此處是透過比較原始『贏家』研究的 *平均* 觀察效應量，與它們對應的重複研究的 *平均* 觀察效應量來觀察。您會發現，重複研究的平均效應量通常會比原始『贏家』研究的平均效應量更接近真實的效應量（在 H_A 情境下）或零（在 H_0 情境下）。"),
                 p(em("在情境一 (對立假設 H_A 為真) 中："), "比較原始「贏家」研究的平均 d，與它們對應的「重複研究」的平均 d。後者的平均 d 通常較低，更接近真實效應大小。"),
...
server <- function(input, output, session) {
  ...
  output$p_value_distribution_plot <- renderPlot({
    ...
    # *** UPDATE: Use after_stat(density) instead of ..density.. ***
    ggplot(plot_data, aes(x = p_value, fill = Hypothesis)) +
      # Use geom_histogram, map y to density using after_stat(), set binwidth and boundary
      # Use position="identity" to overlay histograms with transparency
      geom_histogram(aes(y = after_stat(density)), binwidth = 0.05, boundary = 0, alpha = 0.7, position = "identity", na.rm = TRUE) +
      ...
      theme_minimal(base_family = "sans") + # Using a common sans-serif font
      theme(
